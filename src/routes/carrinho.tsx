  ...
  if (items.length === 0) {
    return (
      <div className="container-luxe py-32 text-center">
        ...
        <h1 className="font-display text-4xl text-onyx">Seu carrinho está vazio</h1>
  ...
  return (
    <div className="container-luxe py-16">
      <div className="text-center mb-12">
        ...
        <h1 className="font-display text-5xl text-onyx">Carrinho</h1>
      ...
      <div className="grid lg:grid-cols-[1fr_400px] gap-12">
        <div className="space-y-4">
          {items.map(({ product, qty }) => (
            <div key={product.id} className="flex gap-5 p-5 border border-border/60 rounded-sm bg-card/30">
              <Link to="/produto/$slug" params={{ slug: product.slug }} className="w-24 h-24 sm:w-32 sm:h-32 shrink-0 overflow-hidden rounded-sm bg-ivory-soft/40">
                <img src={product.images[0]} alt={product.name} className="h-full w-full object-cover" />
              </Link>
              <div className="flex-1 min-w-0 flex flex-col">
                <div className="eyebrow">{product.categoryLabel}</div>
                <Link to="/produto/$slug" params={{ slug: product.slug }} className="font-display text-lg text-onyx truncate hover:text-marguerite">
                  {product.name}
                </Link>
                <div className="text-sm text-muted-foreground mt-1">{formatBRL(product.price)} cada</div>
                <div className="mt-auto flex items-center justify-between gap-3 pt-3">
                  <div className="flex items-center border border-border rounded-sm">
                    <button onClick={() => cart.setQty(product.id, qty - 1)} className="grid h-9 w-9 place-items-center text-onyx hover:text-marguerite">
                      <Minus className="h-3 w-3" />
                    </button>
                    <span className="w-10 text-center text-sm text-onyx">{qty}</span>
                    <button onClick={() => cart.setQty(product.id, qty + 1)} className="grid h-9 w-9 place-items-center text-onyx hover:text-marguerite">
                      <Plus className="h-3 w-3" />
                    </button>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-onyx font-medium">{formatBRL(qty * product.price)}</span>
        ...
        <aside className="h-fit lg:sticky lg:top-28 p-6 border border-border/60 rounded-sm bg-card/30 space-y-5">
          <h2 className="font-display text-2xl text-onyx">Resumo</h2>
          <div className="hairline" />
          <div className="space-y-3 text-sm">
            <div className="flex justify-between text-onyx/80"><span>Subtotal</span><span>{formatBRL(subtotal)}</span></div>
            <div className="flex justify-between text-onyx/80"><span>Frete</span><span className="text-marguerite">A calcular</span></div>
          </div>
          <div className="hairline" />
